...
const AppContent: React.FC = () => {
  ...
  const handleSavePMReport = createHandler((curr, data: PMReportData) => {
    const reports = curr.pmReportData || [];
    const idx = reports.findIndex(r => r.id === data.id);
    const next = [...reports];
    if (idx >= 0) next[idx] = data; else next.push(data);
    return { ...curr, pmReportData: next };
  });
  const handleDeletePMReport = createHandler((curr, id: string) => ({
    ...curr,
  ...
  return (
    <div className="flex bg-gray-50 dark:bg-gray-950 min-h-screen font-sans transition-colors duration-200">
      ...
      <main className="flex-1 ml-64 flex flex-col">
        ...
        {/* Content */}
        <div className="p-8">
            {activeTab === 'dashboard' && <KPIDashboard teams={viewState.teams} systemMessage={viewState.systemMessage} smartTodos={viewState.smartTodos || []} oneOffQueries={(appState.oneOffQueries || []).filter(q => viewState.teams.some(t => t.id === q.teamId))} />}
            {activeTab === 'management' && <ManagementDashboard teams={viewState.teams} users={viewState.users} reports={viewState.weeklyReports} meetings={viewState.meetings} workingGroups={viewState.workingGroups || []} llmConfig={appState.llmConfig} onUpdateReport={handleUpdateReport} onUpdateTeam={handleUpdateTeam} notifications={appState.notifications || []} currentUserId={appState.currentUser?.id || ''} onMarkNotificationSeen={handleMarkNotificationSeen} oneOffQueries={appState.oneOffQueries || []} />}
